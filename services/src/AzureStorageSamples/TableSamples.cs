...
namespace AzureStorageSample.Tests
{
    public class TableSamples : TestBase
    {
        ...
        public async Task S3_QueryEntitiesAsync()
        {
            ...
            {
                _testOutputHelper.WriteLine($"{item.GetString("Product")}: {item.GetDouble("Price")}");
                count++;
            }
            ...
            await foreach (Page<TableEntity> page in queryResultsMaxPerPage.AsPages())
            {
                ...
                {
                    _testOutputHelper.WriteLine($"{item.GetString("Product")} inventoried: {item.GetInt32("Quantity")}");
                }
            }
        ...
        internal sealed class CustomerEntity : ITableEntity
        {
        ...
        internal sealed class OfficeSupplyEntity : ITableEntity
        {
