...
namespace WebApi.Demo
{
    public class QueueListenerBackgroundService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            await _tableServiceClient.CreateTableIfNotExistsAsync(TableName, stoppingToken);
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    while (await HasMessages())
                    {
                        ...
                        QueueMessage[] receivedMessages = _queueClient.ReceiveMessages(10, TimeSpan.FromSeconds(5), stoppingToken);
